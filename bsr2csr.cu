...
int randomBSRMatrix(int mb, int nb, int blockDim, float p, int** hostBsrRowPtr,
                    int** hostBsrColInd, float** hostBsrVal, float minVal = -10,
                    float maxVal = 10) {
...
int main(int argc, char* argv[]) {
  ...
  nnzb = randomBSRMatrix(mb, nb, blockDim, p, &hostBsrRowPtr, &hostBsrColInd,
                         &hostBsrVal);
  ...
  cudaStat1 =
      cudaMemcpy(bsrRowPtr, hostBsrRowPtr, (size_t)((mb + 1) * sizeof(int)),
                 cudaMemcpyHostToDevice);
  cudaStat2 = cudaMemcpy(bsrColInd, hostBsrColInd, (size_t)(nnzb * sizeof(int)),
                         cudaMemcpyHostToDevice);
  cudaStat3 = cudaMemcpy(bsrVal, hostBsrVal,
                         (size_t)(nnzb * blockDim * blockDim * sizeof(float)),
                         cudaMemcpyHostToDevice);
  if (cudaStat1 != cudaSuccess || cudaStat2 != cudaSuccess ||
      cudaStat3 != cudaSuccess) {
  ...
  status = cusparseSbsr2csr(handle, CUSPARSE_DIRECTION_ROW, mb, nb, bsrDescr,
                            bsrVal, bsrRowPtr, bsrColInd, blockDim, csrDescr,
