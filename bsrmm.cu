...
int main() {
  ...
  if ((!hostBsrRowPtr) || (!hostBsrColInd) || (!hostBsrVal) || (!yHostPtr) ||
      (!zHostPtr)) {
  ...
  cudaStat3 = cudaMemcpy(bsrVal, hostBsrVal,
                         ...
                         cudaMemcpyHostToDevice);
  cudaStat4 = cudaMemcpy(y, yHostPtr, (size_t)(k * n * sizeof(float)),
                         cudaMemcpyHostToDevice);
  ...
  cudaStat1 = cudaMemcpy(zHostPtr, z, (size_t)(m * n * sizeof(float)),
