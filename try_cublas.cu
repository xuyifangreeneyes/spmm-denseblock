...
int main() {
  ...
  // HANDLE_CUBLAS_ERROR( cublasSetMatrix(m, k, sizeof(float), hostA, m, A, m),
  // "cublasSetMatrix1 failed" );
  // HANDLE_CUBLAS_ERROR( cublasSetMatrix(k, n, sizeof(float), hostB, k, B, k),
  // "cublasSetMatrix2 failed" );
  // HANDLE_CUBLAS_ERROR( cublasSetMatrix(m, n, sizeof(float), hostC, m, C, m),
  // "cublasSetMatrix3 failed" );

  HANDLE_ERROR(cudaMemcpy(A, hostA, (size_t)(m * k * sizeof(float)),
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(B, hostB, (size_t)(k * n * sizeof(float)),
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemset((void*)C, 0, m * n * sizeof(float)));

  HANDLE_CUBLAS_ERROR(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k,
                                  &alpha, A, m, B, k, &beta, C, m),
                      "cublasSgemm failed");

  // cublasGetMatrix(m, n, sizeof(float), C, m, hostC, m);
  HANDLE_ERROR(cudaMemcpy(hostC, C, (size_t)(m * n * sizeof(float)),
