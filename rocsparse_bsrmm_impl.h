...
__device__ __forceinline__ float myfma(float x, float y, float z) {
  return fmaf(x, y, z);
}
__device__ __forceinline__ double myfma(double x, double y, double z) {
  return fma(x, y, z);
}

template <typename T, int BLOCKSIZE, int WF_SIZE, int BSR_BLOCK_DIM>
static __device__ void bsrmmnn_small_blockdim_device(
  ...
  for (int block_row = gid / (WF_SIZE * BSR_BLOCK_DIM); block_row < Mb;
    ...
    for (int j = block_row_start; j < block_row_end; j += WF_SIZE) {
      ...
      if (direction == CUSPARSE_DIRECTION_ROW) {
        ...
        //         = (k < block_row_end) ? bsr_val[BSR_BLOCK_DIM * BSR_BLOCK_DIM
        //         * k
        //                                               + BSR_BLOCK_DIM *
        //                                               local_row + l]
        //                               : static_cast<T>(0);
...
template <typename T, int BLOCKSIZE, int WF_SIZE, int BSR_BLOCK_DIM>
static __device__ void bsrmmnt_small_blockdim_device(
    cusparseDirection_t direction, int Mb, int N, T alpha,
    const int* __restrict__ bsr_row_ptr, const int* __restrict__ bsr_col_ind,
    const T* __restrict__ bsr_val, const T* __restrict__ B, int ldb, T beta,
    T* __restrict__ C, int ldc, int idx_base) {
  ...
  for (int l = 0; l < N; l += WF_SIZE) {
    ...
    for (int j = block_row_start; j < block_row_end; j += WF_SIZE) {
      ...
      shared_col[wid][lid] =
          (k < block_row_end) ? N * BSR_BLOCK_DIM * (bsr_col_ind[k] - idx_base)
                              : 0;
      ...
      if (col < N) {
        for (int i = 0; i < WF_SIZE; ++i) {
          ...
          // for(int p = 0; p < BSR_BLOCK_DIM; p++)
          // {
          //     T val_B = __ldg(B + col + N * p + shared_col[wid][i]);
          //     sum = myfma(shared_val[wid][PADDED_BSR_BLOCK_DIM * i + p],
          //     val_B, sum);
          // }
          // as unrolled loop.
          T val_B = __ldg(B + col + shared_col[wid][i]);
          sum = myfma(shared_val[wid][PADDED_BSR_BLOCK_DIM * i], val_B, sum);
          if (BSR_BLOCK_DIM >= 2) {
            val_B = __ldg(B + col + N * 1 + shared_col[wid][i]);
            sum = myfma(shared_val[wid][PADDED_BSR_BLOCK_DIM * i + 1], val_B,
                        sum);
          }
          if (BSR_BLOCK_DIM >= 3) {
            val_B = __ldg(B + col + N * 2 + shared_col[wid][i]);
            sum = myfma(shared_val[wid][PADDED_BSR_BLOCK_DIM * i + 2], val_B,
                        sum);
          }
          if (BSR_BLOCK_DIM >= 4) {
            val_B = __ldg(B + col + N * 3 + shared_col[wid][i]);
            sum = myfma(shared_val[wid][PADDED_BSR_BLOCK_DIM * i + 3], val_B,
                        sum);
          }
        }
      }
...
static __device__ void bsrmm_large_blockdim_device(
    cusparseDirection_t direction, cusparseOperation_t trans_B, int Mb, int N,
    T alpha, const int* __restrict__ bsr_row_ptr,
    const int* __restrict__ bsr_col_ind, const T* __restrict__ bsr_val,
    int block_dim, const T* __restrict__ B, int ldb, T beta, T* __restrict__ C,
  ...
  for (int k = block_row_start; k < block_row_end; k++) {
    ...
    for (int j = 0; j < block_dim; j++) {
      sum = myfma(shared_A[BSR_BLOCK_DIM * j + tidx],
                  shared_B[BSR_BLOCK_DIM * tidy + j], sum);
    }
...
template <typename T, int BSR_BLOCK_DIM, int BLK_SIZE_Y>
static __device__ void bsrmm_general_blockdim_device(
    cusparseDirection_t direction, cusparseOperation_t trans_B, int Mb, int N,
    T alpha, const int* __restrict__ bsr_row_ptr,
    const int* __restrict__ bsr_col_ind, const T* __restrict__ bsr_val,
    int block_dim, const T* __restrict__ B, int ldb, T beta, T* __restrict__ C,
