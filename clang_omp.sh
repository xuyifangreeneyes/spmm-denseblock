...
/tmp/var/target/linux-desktop-glibc_2_11_3-x64/ncu --export /tmp/var/nsight_output.txt --force-overwrite --target-processes all --kernel-regex-base function --launch-skip-before-match 0 --section ComputeWorkloadAnalysis --section InstructionStats --section LaunchStats --section MemoryWorkloadAnalysis --section MemoryWorkloadAnalysis_Chart --section Occupancy --section SchedulerStats --section SpeedOfLight --section WarpStateStats --sampling-interval auto --sampling-max-passes 5 --sampling-buffer-size 33554432 --profile-from-start 0 --cache-control all --clock-control base --apply-rules yes ./run_csrmm arxiv_original 64 cusparseScsrmm 0