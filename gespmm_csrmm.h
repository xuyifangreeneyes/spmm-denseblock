...
__global__ void spmm_test0(int A_nrows, int B_ncols, int* A_csrRowPtr,
                           int* A_csrColInd, T* A_csrVal, T* B_dnVal,
                           T* C_dnVal) {
...
template <typename T>
__global__ void spmm_test3(int A_nrows, int B_ncols, int* A_csrRowPtr,
                           int* A_csrColInd, T* A_csrVal, T* B_dnVal,
                           T* C_dnVal) {
  extern __shared__ int sh[];
  int* colInd_sh = sh;
...
template <typename T>
__global__ void spmm_test4(int A_nrows, int B_ncols, int* A_csrRowPtr,
                           int* A_csrColInd, T* A_csrVal, T* B_dnVal,
                           T* C_dnVal) {
  extern __shared__ int sh[];
  int* colInd_sh = sh;
  ...
  if (rid < A_nrows) {
    ...
    int ptr = lb + threadIdx.x;
    int offset;
    T acc1 = 0, acc2 = 0, acc3 = 0, acc4 = 0, acc5 = 0, acc6 = 0, acc7 = 0,
      acc8 = 0, val;
...
template <typename T>
void spmmWrapper(int method, int tile_row, int A_nrows, int B_ncols,
                 int* A_rowPtr, int* A_colInd, T* A_val, T* B, T* C) {
  switch (method) {
    case 0:
      if (B_ncols > 32) {
        spmm_test0<T><<<dim3((A_nrows + tile_row - 1) / tile_row,
                             (B_ncols + 31) / 32, 1),
                        dim3(32, tile_row, 1), 0, 0>>>(
            A_nrows, B_ncols, A_rowPtr, A_colInd, A_val, B, C);
      } else {
        spmm_test0<T><<<dim3((A_nrows + tile_row - 1) / tile_row, 1, 1),
                        dim3(B_ncols, tile_row, 1), 0, 0>>>(
            A_nrows, B_ncols, A_rowPtr, A_colInd, A_val, B, C);
      }
      break;
    case 1:
      spmm_test1<T><<<
          dim3((A_nrows + tile_row - 1) / tile_row, (B_ncols + 31) / 32, 1),
          dim3(32, tile_row, 1), 32 * tile_row*(sizeof(int) + sizeof(T)), 0>>>(
          A_nrows, B_ncols, A_rowPtr, A_colInd, A_val, B, C);
      break;
    case 2:
      spmm_test2<T><<<
          dim3((A_nrows + tile_row - 1) / tile_row, (B_ncols + 63) / 64, 1),
          dim3(32, tile_row, 1), 32 * tile_row*(sizeof(int) + sizeof(T)), 0>>>(
          A_nrows, B_ncols, A_rowPtr, A_colInd, A_val, B, C);
      break;
    case 3:
      spmm_test3<T><<<
          dim3((A_nrows + tile_row - 1) / tile_row, (B_ncols + 127) / 128, 1),
          dim3(32, tile_row, 1), 32 * tile_row*(sizeof(int) + sizeof(T)), 0>>>(
          A_nrows, B_ncols, A_rowPtr, A_colInd, A_val, B, C);
      break;
    case 4:
      spmm_test4<T><<<
          dim3((A_nrows + tile_row - 1) / tile_row, (B_ncols + 255) / 256, 1),
          dim3(32, tile_row, 1), 32 * tile_row*(sizeof(int) + sizeof(T)), 0>>>(
          A_nrows, B_ncols, A_rowPtr, A_colInd, A_val, B, C);
      break;
  }
...
template <typename T>
void gespmm_csrmm(int A_nrows, int B_ncols, int* A_rowPtr, int* A_colInd,
                  T* A_val, T* B, T* C) {
