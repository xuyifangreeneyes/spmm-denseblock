...
void readCSRMatrix(int m, int n, int nnz, int** hostCsrRowPtr,
...
int main() {
  ...
  status =
      cusparseScsrmm2(handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                      CUSPARSE_OPERATION_TRANSPOSE, m, dim, n, nnz, &alpha,
                      descr, csrVal, csrRowPtr, csrColInd, y, dim, &beta, z, m);
  ...
  cudaStat1 = cudaMemcpy(zHostPtr, z, (size_t)(m * dim * sizeof(float)),
