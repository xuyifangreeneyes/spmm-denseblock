...
int main(int argc, char* argv[]) {
  ...
  printf("p = %f blockDim = %d dim = %d bsrmmImpl = %s transposeB = %d\n", p,
         blockDim, dim, bsrmmImpl.c_str(), transposeB);
  ...
  // &hostBsrVal, -1, 1, true);
  nnzb = readAndFillBSRMatrix(mb, nb, blockDim, p, &hostBsrRowPtr,
                              &hostBsrColInd, &hostBsrVal);
  ...
  if (bsrmmImpl == "rocsparse") {
    HANDLE_CUSPARSE_ERROR(rocsparse_bsrmm_template<float>(
        ...
        transB, mb, dim, nb, nnzb, fone, descr, bsrVal, bsrRowPtr, bsrColInd,
        blockDim, y, ldb, fzero, z, m));
