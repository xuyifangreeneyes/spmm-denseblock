...
int main() {
  ...
  cudaStat2 = cudaMemcpy(cooColIndex, cooColIndexHostPtr,
                         (size_t)(nnz * sizeof(int)), cudaMemcpyHostToDevice);
  cudaStat3 = cudaMemcpy(cooVal, cooValHostPtr, (size_t)(nnz * sizeof(float)),
                         cudaMemcpyHostToDevice);
  cudaStat4 = cudaMemcpy(y, yHostPtr, (size_t)(2 * n * sizeof(float)),
                         cudaMemcpyHostToDevice);
  ...
  status = cusparseScsrmm(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, 2, n,
                          nnz, &fone, descr, cooVal, csrRowPtr, cooColIndex, y,
                          n, &fzero, z, n);
  if (status != CUSPARSE_STATUS_SUCCESS) {
  ...
  cudaStat1 = cudaMemcpy(zHostPtr, z, (size_t)(2 * n * sizeof(float)),
