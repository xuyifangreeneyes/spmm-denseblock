...
int main() {
    ...
    // HANDLE_ERROR( cudaMemcpy(alpha, hostAlpha, (size_t) sizeof(float), cudaMemcpyHostToDevice) );
    // HANDLE_ERROR( cudaMemcpy(beta, hostBeta, (size_t) sizeof(float), cudaMemcpyHostToDevice) );
